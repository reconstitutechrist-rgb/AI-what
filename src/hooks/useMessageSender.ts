...
import type { ChatMessage, GeneratedComponent, StagePlan, PendingDiff, CurrentStagePlan } from '@/types/aiBuilderTypes';
...
export interface UseMessageSenderReturn {
  ...
  isReadyToStartPhase: (lastMessage: ChatMessage | undefined, stagePlan: StagePlan | null) => boolean;
  /** Check if user is ready to continue to next phase */
  isReadyToContinuePhase: (lastMessage: ChatMessage | undefined, stagePlan: StagePlan | null) => boolean;
...
export function useMessageSender(_options: UseMessageSenderOptions): UseMessageSenderReturn {
  ...
  const isQuestion = useCallback((input: string): boolean => {
    ...
    const hasBuildWords = BUILD_INDICATORS.some(indicator => 
      lowerInput.includes(indicator)
    );
  ...
  const isCodeRequest = useCallback((input: string): boolean => {
    const lowerInput = input.toLowerCase();
    return CODE_REQUEST_INDICATORS.some(indicator => 
      lowerInput.includes(indicator)
    );
  }, []);
   ...
   * Detect if input is a complex modification
   */
  const isComplexModification = useCallback((input: string, hasCurrentComponent: boolean): boolean => {
    if (!hasCurrentComponent) return false;
    
    const lowerInput = input.toLowerCase();
    return COMPLEX_MODIFICATION_INDICATORS.some(indicator => 
      lowerInput.includes(indicator)
    );
  }, []);
   ...
   * Check if user is at a phase start point
   */
  const isReadyToStartPhase = useCallback((lastMessage: ChatMessage | undefined, stagePlan: StagePlan | null): boolean => {
   ...
   * Check if user is ready to continue to next phase
   */
  const isReadyToContinuePhase = useCallback((lastMessage: ChatMessage | undefined, stagePlan: StagePlan | null): boolean => {
    if (!lastMessage || !stagePlan) return false;
   ...
   * Get progress messages based on request type
   */
  const getProgressMessages = useCallback((isQuestionType: boolean, isModification: boolean): string[] => {
    if (isQuestionType) {
      return [
