...
const PLACEHOLDER_PATTERNS = {
  ...
  genericPlaceholder: /['"]https?:\/\/(?:via\.placeholder\.com|placeholder\.com|placehold\.co)[^'"]*['"]/g,
...
export function injectImageUrls(
  ...
  for (const file of files) {
    ...
    if (hasPlaceholders) {
      ...
      if (fileModified && isReactComponent(file.path) && !content.includes("from '@/constants/images'")) {
...
function replaceHeroPlaceholders(content: string, heroUrl: string): string {
  ...
  result = result.replace(
    /(['"])https?:\/\/via\.placeholder\.com\/\d+x\d+[^'"]*\1/g,
    (match) => {
...
function addImageImport(content: string): string {
  const importStatement = "import { HERO_IMAGE, CARD_IMAGES, getCardImage } from '@/constants/images';\n";
