  ...
  return (
    <button
      ...
      className={`px-3 py-1.5 text-xs font-medium transition-colors ${
        active
          ? 'text-white bg-slate-700 border-b-2 border-garden-500'
...
function highlightCSS(code: string): string {
  return (
    code
      ...
      // CSS variables
      .replace(/(--[\w-]+)/g, '<span class="text-gold-400">$1</span>')
      // Properties
        ...
        <div className="flex items-center gap-2 pr-2">
          <button
            ...
            className={`p-1 rounded text-xs transition-colors ${
              showLineNumbers
                ? 'text-garden-400 bg-garden-500/10'
...
`
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="px-2 py-1 text-xs text-white bg-garden-600 hover:bg-garden-500 disabled:bg-garden-600/50 rounded transition-colors flex items-center gap-1"
    >
      {isLoading ? (
