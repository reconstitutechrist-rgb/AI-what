  ...
  return (
    <div className="w-96 border-l border-zinc-800 flex flex-col bg-zinc-900/50">
      ...
      <div className="p-4 border-b border-zinc-800">
        ...
        <p className="text-sm text-zinc-400 mt-1">Review the generated architecture</p>
      </div>
          ...
          {architectureSpec.database?.prismaSchema ? (
            <div className="bg-zinc-950 rounded border border-zinc-800 overflow-hidden">
              <div className="px-3 py-1.5 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between">
                <span className="text-xs text-zinc-400 font-mono">schema.prisma</span>
              </div>
              <pre className="p-3 text-xs text-zinc-300 font-mono overflow-x-auto max-h-64 overflow-y-auto">
                {architectureSpec.database.prismaSchema}
              </pre>
            </div>
          ) : (
            <div className="space-y-2">
              {architectureSpec.database?.tables?.map((table) => (
                <div key={table.name} className="p-2 bg-zinc-800/50 rounded">
                  <p className="text-sm font-medium text-zinc-200">{table.name}</p>
                  <p className="text-xs text-zinc-400 mt-0.5">{table.fields?.length || 0} fields</p>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="space-y-1">
            {architectureSpec.api?.routes?.map((route, idx) => (
              <div
                ...
                className="p-2 bg-zinc-800/50 rounded flex items-start gap-2"
              >
                <span
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-mono text-zinc-200 truncate">{route.path}</p>
                  {route.description && (
                    <p className="text-xs text-zinc-400 mt-0.5">{route.description}</p>
                  )}
                </div>
              </div>
            ))}
        ...
        {architectureSpec.auth && (
          ...
          >
            <div className="space-y-3">
              <div>
                <label className="text-xs text-zinc-500 uppercase tracking-wide">Strategy</label>
              ...
              {architectureSpec.auth.providers && architectureSpec.auth.providers.length > 0 && (
                <div>
                  <label className="text-xs text-zinc-500 uppercase tracking-wide">Providers</label>
                ...
                <div>
                  ...
                  <div className="mt-1 space-y-1">
                    {architectureSpec.auth.rbac.roles.map((role) => {
                      ...
                      return (
                        <div key={role.name} className="p-2 bg-zinc-800/50 rounded">
                          <p className="text-sm font-medium text-zinc-200">{role.name}</p>
              ...
              <div className="space-y-2">
                {architectureSpec.architectureReasoning.decisions.map((decision, idx) => (
                  <div key={idx} className="p-2 bg-zinc-800/50 rounded">
                    <p className="text-xs text-zinc-500 uppercase tracking-wide">{decision.area}</p>
                    <p className="text-sm text-zinc-200 mt-0.5">{decision.decision}</p>
                    {decision.reasoning && (
                      <p className="text-xs text-zinc-400 mt-1 italic">{decision.reasoning}</p>
                    )}
                  </div>
                ))}
              </div>
    ...
    <div className="border-b border-zinc-800">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {badge && (
            <span className="px-1.5 py-0.5 bg-zinc-700 text-zinc-300 rounded text-xs">{badge}</span>
          )}
        </div>
        <svg
